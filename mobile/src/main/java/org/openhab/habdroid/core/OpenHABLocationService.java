 ...
 */
public class OpenHABLocationService extends ContinuingIntentService implements OpenHABTrackerReceiver {
    ...
    private void initHttpClient() {
        ...
        username = mSettings.getString(Constants.PREFERENCE_USERNAME, null);
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (intent.hasExtra(OPENHAB_BASE_URL_EXTRA)) {
            ...
            mOpenHABTracker = new OpenHABTracker(OpenHABLocationService.this, getString(R.string.openhab_service_type), false);
    ...
    public void onOpenHABTracked(String baseUrl, String message) {
        ...
        while (!mBufferedIntents.isEmpty()) {
            processLocationIntent(mBufferedIntents.poll());
        }
    ...
    private void processLocationIntent(Intent data) {
        Log.d("INTENT_LOCATION","onLocationChanged");
        final Location location = data.getParcelableExtra(LocationServices.FusedLocationApi.KEY_LOCATION_CHANGED);
        String mLastUpdateTime = DateFormat.getTimeInstance().format(new Date());
        TelephonyManager tm = (TelephonyManager)getSystemService(TELEPHONY_SERVICE);
        String number = tm.getDeviceId();
        sendItemCommand("LOCATION",String.valueOf(location.getLatitude())+","+
                String.valueOf(location.getLongitude())+","+number+","+username);
    }
    ...
    private void performHttpPost(final String itemName, final StringEntity command) {
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            @Override
            public void run() {
                mAsyncHttpClient.post(OpenHABLocationService.this, mOpenHABBaseUrl + "rest/items/" + itemName,
